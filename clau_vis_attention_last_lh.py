...
class RegionalAttentionAnalyzer:
    ...
    def setup_directories(self):
        """设置必要的目录结构"""
        self.plots_dir = Path('vis_clau_ablation_study_lh_last')
    ...
    def analyze_regional_attention_with_atlas(self, baseline_maps, no_feature_maps, data_arr_lh):
        ...
        # 1. 读取和准备DK Atlas数据
        annot_path = '/projects/0/einf1049/scratch/jouyang/lh.aparc.annot'
    ...
    def _plot_regional_analysis(self, results):
        ...
        ax.set_title('Regional Attention Analysis (Spatial Attention Module, Left-semi) - Top 15 Regions')
        ax.set_xticks(x)
...
def create_brain_atlas_visualization(label_map, label_names):
    ...
    plt.axis('off')
    plt.title('2D Left-semi Brain Regions Visualization with DK Atlas', fontsize=16, pad=20)
    ...
    # 保存高清图像
    plt.savefig('/home/jouyang1/vis_clau_ablation_study_lh_last/attention/brain_atlas_clear.png',
                dpi=300,
                bbox_inches='tight',
                facecolor='white',
                edgecolor='none')
...
def main():
    """主函数运行完整的区域分析"""
    # 1. 设置路径
    base_dir = Path('/home/jouyang1/ablation_study_lh_results_20250207_172701/ablation_study_20250207_172702')  # 替换为你的实际路径
    ...
    # 3. 加载坐标数据
    head = open("/projects/0/einf1049/scratch/jouyang/GenR_mri/lh.fsaverage.sphere.cortex.mask.label", "r")
    ...
    try:
        ...
        # 6. 保存结果
        output_file = Path('vis_clau_ablation_study_lh_last') / 'analysis_results' / 'regional_attention_analysis.npy'
