...
from experiment_primary_updated_rh import ModelExperiments
from train_precdiction_model_images_improved_rh import BrainADHDModel, BrainMapDataset
import os
import torch
import time
import numpy as np
...
def load_data(image_path, phenotype_path, use_mmap=True, test_mode=False, test_ratio=0.1):
    ...
    try:
        ...
        if test_mode:
            ...
            logging.info(f"rh: Test mode: Using {test_ratio*100}% of data")
            logging.info(f"rh: Selected {test_samples} samples from {total_samples} total samples")
        
        logging.info(f"rh: Loaded data shape: Image {image_data.shape}, Phenotype {phenotype_data.shape}")
        return image_data, phenotype_data
        
    except Exception as e:
        logging.error(f"rh: Error loading data: {str(e)}")
        raise
...
def create_summary_report(results, results_dir):
    ...
    with open(report_path, 'w') as f:
        f.write("=" * 50 + "\n")
        f.write("rh - Experiment Summary Report\n")
        f.write("=" * 50 + "\n\n")
        
        # 实验参数
        f.write("rh - Experiment Parameters:\n")
        f.write("-" * 30 + "\n")
        f.write(f"rh - Test Mode: {results['test_mode']}\n")
        f.write(f"rh - Total Time: {results['total_time']/3600:.2f} hours\n\n")
        
        # Sensitivity Analysis - 分别显示sum_att和sum_agg的结果
        f.write("1. rh - Sensitivity Analysis\n")
        f.write("-" * 30 + "\n")
        for score_name in ['sum_att', 'sum_agg']:
            ...
            for noise_type in ['gaussian_noise', 'random_masking']:
                ...
                for measurement in results['sensitivity'][score_name][noise_type]:
                    f.write(f"rh - Noise level: {measurement['noise_level']}\n")
                    f.write(f"rh - Mean change: {measurement['mean_change']:.4f}\n")
                    f.write(f"rh - Standard deviation: {measurement['std_dev']:.4f}\n")
                    f.write(f"rh - ICC: {measurement['icc']:.4f}\n")
                    f.write(f"rh - ICC p-value: {measurement['icc_p_value']:.4f}\n")
        
        # Sample Size Impact
        f.write("\n2. rh - Sample Size Impact\n")
        f.write("-" * 30 + "\n")
        for score_name in ['sum_att', 'sum_agg']:
            f.write(f"\n{score_name.upper()}:\n")
            for result in results['sample_size'][score_name]:
                f.write(f"\nSample size: {result['sample_size']*100}%\n")
                f.write(f"rh - MSE: {result['mse_mean']:.4f} (±{result['mse_std']:.4f})\n")
                f.write(f"rh - R2: {result['r2_mean']:.4f} (±{result['r2_std']:.4f})\n")
                f.write(f"rh - Correlation: {result['correlation_mean']:.4f} (±{result['correlation_std']:.4f})\n")
        
        # Stability Analysis
        f.write("\n3. rh - Stability Analysis\n")
        f.write("-" * 30 + "\n")
        for score_name in ['sum_att', 'sum_agg']:
            f.write(f"\n{score_name.upper()}:\n")
            metrics = results['stability'][1][score_name]  # Using the final metrics
            f.write(f"rh - MSE: {metrics['mse_mean']:.4f} (±{metrics['mse_std']:.4f})\n")
            f.write(f"rh - R2: {metrics['r2_mean']:.4f} (±{metrics['r2_std']:.4f})\n")
            f.write(f"rh - Correlation: {metrics['correlation_mean']:.4f} (±{metrics['correlation_std']:.4f})\n")
        
        # Computational Assessment
        f.write("\n4. rh - Computational Assessment\n")
        f.write("-" * 30 + "\n")
        comp = results['computational']
        f.write(f"rh - Training time per epoch: {comp['training_metrics']['avg_time_per_epoch']:.2f}s\n")
        f.write(f"rh - Average inference time: {comp['inference_metrics']['avg_inference_time']:.4f}s\n")
        f.write(f"rh - GPU memory usage: {comp['memory_metrics']['gpu_memory']['allocated']:.2f} GB\n")
        f.write(f"rh - Model parameters: {comp['model_info']['total_parameters']:,}\n")

def main(test_mode=False):

    print(f"rh - Test mode:",test_mode)
    ...
    base_dir = '/home/jouyang1/test_experiment_results_rh' if test_mode else '/home/jouyang1/experiment_results_primary_rh'
    experiment_dir = os.path.join(base_dir, f'rh_experiment_{timestamp}')
    ...
    # 尝试创建所有必要的目录
        ...
        for subdir in ['data', 'visualizations', 'logs', 'metrics']:
            ...
            logging.info(f"rh - Created subdirectory: {subdir_path}")
    except Exception as e:
        logging.error(f"rh - Error creating directories: {e}")
        raise

    logs_dir = Path(os.path.join(experiment_dir, 'logs'))

    # verify the folders
    logging.info(f"rh - Experiment directory exists: {os.path.exists(experiment_dir)}")
    logging.info(f"rh - Experiment directory contents: {os.listdir(base_dir)}")
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(logs_dir / "rh_experiments.log"), #has to be path objective to use / 
            logging.StreamHandler()
        ]
    )
    # 打印实验配置
    logging.info("rh - Experiment Configuration:")
    logging.info(f"rh - Test Mode: {test_mode}")
    logging.info(f"rh - Batch Size: {batch_size}")
    logging.info(f"rh - Number of Workers: {num_workers}")
    logging.info(f"rh - Base Epochs: {5 if test_mode else 30}")

    # Initialize device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logging.info(f"rh - Using device: {device}")
    ...
    try: 
        if not progress.get('sensitivity_test_completed', False):
            ...
            logging.info("1.1 rh - Starting sensitivity tests...")
            model = BrainADHDModel(num_phenotypes=3).to(device)
            model.load_state_dict(torch.load('all_rh_best_model_improved.pth', weights_only=True)) 
            
            sensitivity_info = experiments.sensitivity_test(
                'all_rh_best_model_improved.pth',
                test_loader,
            ...
            with open(progress['sensitivity_info']['metrics_file'], 'r') as f:
                sensitivity_results = json.load(f)['results']
            logging.info("Skipping rh - 1.1 sensitivity tests (already completed)")
        

        if not progress.get('sample_size_impact_completed', False):
    
            # 1.2 Sample Size Impact
            logging.info("1.2 rh - Starting sample size impact analysis...")
            ...
            with open(progress['sample_size_info']['metrics_file'], 'r') as f:
                sample_size_results = json.load(f)['results']
            logging.info("Skipping rh - 1.2 sample size impact analysis (already completed)")


        if not progress.get('stability_evaluation_completed', False):
            # 2. Stability Evaluation
            logging.info("2 rh - Starting stability evaluation...")
            ...
            with open(progress['stability_info']['metrics_file'], 'r') as f:
                stability_data = json.load(f)
                stability_results = stability_data['results'], stability_data['final_metrics']
            logging.info("Skipping rh - 2. stability evaluation (already completed)")

        if not progress.get('computational_assessment_completed', False):

            # 3. Computational Assessment
            logging.info("3 rh - Starting computational assessment...")
            ...
            with open(progress['computational_info']['metrics_file'], 'r') as f:
                comp_results = json.load(f)
            logging.info("Skipping rh - 3. computational assessment (already completed)")

    
        # 记录总运行时间
        total_time = time.time() - start_time
        logging.info(f"rh - Total experiment time: {total_time/3600:.2f} hours")
        ...
        logging.error(f"rh - Experiment interrupted: {str(e)}")
