...
class BrainADHDModel(nn.Module):
    ...
    def forward(self, x, phenotypes):
        ...
        print(f"before frontend:{x.shape}")
        x = self.frontend(x)
        print(f"after frontend:{x.shape}")
        # Apply attention
        ...
        x = self.avg_pool(x)
        x = x.view(x.size(0), -1)
        print(f"after global pooling anf flatten:{x.shape}")
        # Process phenotype data
        phenotype_features = self.phenotype_encoder(phenotypes)
        print(f"phenotype shape:{phenotype_features.shape}")
        # Combine features
        ...
        print(f"weighted_features shape: {weighted_features.shape}")
        
        # Final prediction
        # 使用加权特征进行预测
        age_pred = self.age_predictor(weighted_features)
        print(f"age_pred shape: {age_pred.shape}")
        attention_pred = self.attention_predictor(weighted_features)
        print(f"attention_pred shape: {attention_pred.shape}")
...
if __name__ == "__main__":
    ...
    output_dir = f'training_all_data_rh_mirror_attmap_{timestamp}'
    ...
    csv_file = os.path.join(results_manager.subdirs['logs'], 'all_rh_training_loss_predictionM1_images_improved.csv')
    ...
    for epoch in range(config['num_epochs']):
        ...
        if val_loss < best_val_loss:
            ...
            # model_path = os.path.join(models_dir, 'all_rh_best_model_improved.pth')
    ...
    # np.save(os.path.join(plots_dir, 'rh_training_history.npy'), train_history)
    ...
    plt.savefig(os.path.join(results_manager.subdirs['plots']['training'], 'all_rh_learning_curve_predict_brainimages_improved.png'))
    plt.close()
    ...
    # 最终评估
    # model_path = os.path.join(results_manager.subdirs['models'], 'all_rh_best_model_improved.pth')
    model_path = os.path.join(results_manager.subdirs['models'], 'all_rh_best_model.pth')
