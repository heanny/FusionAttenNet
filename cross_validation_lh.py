...
def signal_handler(signum, frame):
    ...
    if current_model is not None and current_optimizer is not None:
        logging.info(f"Received the break signal, save the checkpoint for fold {current_fold}...")
        try:
            ...
            logging.info("Checkpoint is saved succefully!")
        except Exception as e:
            logging.error(f"Error occurs when saving the checkpoint: {str(e)}")
    sys.exit(0)
...
if __name__ == "__main__":
    ...
    output_dir = f'cross_validation_lh_{timestamp}'
