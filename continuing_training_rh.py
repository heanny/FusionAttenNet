...
def parse_training_log(log_file_path):
    ...
    train_history = {
        ...
        'train_att_losses': [],
        'train_age_losses': [],
        'learning_rates': []  # 这个可能需要重新收集
    }
    
    # 编译正则表达式模式
    pattern = r"Train Loss: ([\d.]+) \(Attention: ([\d.]+), Age: ([\d.]+)\), Val Loss: ([\d.]+)"
    ...
    try:
        ...
        for match in matches:
            ...
            att_loss = float(match.group(2))
            age_loss = float(match.group(3))
            val_loss = float(match.group(4))
            
            train_history['train_losses'].append(train_loss)
            train_history['train_att_losses'].append(att_loss)
            train_history['train_age_losses'].append(age_loss)
            train_history['val_losses'].append(val_loss)
        
        print(f"Successfully parsed log file. Found {len(train_history['train_losses'])} epochs of data.")
        
        # 检查是否有训练历史文件
        previous_history_path = os.path.join(previous_output_dir, 'plots/training_plots', 'training_history.npy')
        if os.path.exists(previous_history_path):
            try:
                prev_history = np.load(previous_history_path, allow_pickle=True).item()
                if 'learning_rates' in prev_history:
                    train_history['learning_rates'] = list(prev_history['learning_rates'])
                    print(f"Successfully loaded learning rates from previous history. Found {len(train_history['learning_rates'])} entries.")
            except:
                print("Could not load previous training_history.npy")
        
        return train_history, len(train_history['train_losses'])
...
if __name__ == "__main__":
    ...
    def evaluate_model(model, test_loader, device, plots_dir):
        ...
        for i, target_name in enumerate(['sum_att', 'age']):
            ...
            plt.savefig(os.path.join(plots_dir, f'all_rh_test_predictions_{target_name}_brainimages_improved.png'), dpi=300)
            plt.close()
            ...
            np.save(os.path.join(plots_dir, 'rh_test_results_age_att.npy'), results)
    ...
    def save_attention_maps(model, test_loader, plots_dir):
        ...
        with torch.no_grad():
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(plots_dir, 'rh_attention_visualization.png'))
            plt.close()
    ...
    # 设置时间限制（30分钟）
    MAX_TRAINING_TIME = 20 * 60 * 60  # 5.5 hours, 留2分钟做最终评估和可视化
    
    # 获取上一次训练的时间戳目录
    # 注意：需要替换成你的实际时间戳
    previous_timestamp = "20250204_062508"  # 替换成你之前的训练时间戳 20250204_062508
    previous_output_dir = f'training_all_data_rh_mirror_attmap_{previous_timestamp}'


    # 在主代码中
    log_file_path = os.path.join(previous_output_dir, 'logs', 'all_rh_training.log')
    previous_train_history, completed_epochs = parse_training_log(log_file_path)

    if previous_train_history is not None:
        print("Training history rebuilt from logs")
        print(f"Last train loss: {previous_train_history['train_losses'][-1]}")
        print(f"Last val loss: {previous_train_history['val_losses'][-1]}")
        # ... 继续训练的代码 ...
    else:
        print("Could not rebuild training history, starting fresh")
        previous_train_history = {
            'train_losses': [],
            'val_losses': [],
            'train_att_losses': [],
            'train_age_losses': [],
    ...
    output_dir = f'training_all_data_rh_mirror_attmap_continued_{timestamp}'
    ...
    logging.basicConfig(level=logging.INFO,
                    ...
                    handlers=[
                        logging.FileHandler(os.path.join(results_manager.subdirs['logs'], "all_rh_training_continued.log")),
                        logging.StreamHandler()
                    ])
    
    # 加载之前的训练历史和指标
    # previous_history_path = os.path.join(previous_output_dir, 'plots/training_plots', 'training_history.npy')
    # previous_train_history = np.load(previous_history_path, allow_pickle=True).item()
    
    # 继承之前的训练历史
    train_history = {
        'train_losses': list(previous_train_history['train_losses']),
        'val_losses': list(previous_train_history['val_losses']),
        'train_att_losses': list(previous_train_history['train_att_losses']),
        'train_age_losses': list(previous_train_history['train_age_losses']),
        'learning_rates': list(previous_train_history['learning_rates'])
    }
    
    # 获取已完成的epoch数
    completed_epochs = len(previous_train_history['train_losses'])
    logging.info(f"Previously completed {completed_epochs} epochs")
    
    # 设置设备和加载数据
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    image_data, loaded_phenotype_tensor = load_data(
        '/projects/0/einf1049/scratch/jouyang/all_cnn_rh_brainimages.npy',
        '/home/jouyang1/all_normalised_phenotypes_correct.npy',
        use_mmap=True
    )
    
    train_loader, val_loader, test_loader = get_data_loaders(
        image_data, loaded_phenotype_tensor, 
        batch_size=config['batch_size'], 
        num_workers=2
    )
    ...
    previous_model_path = os.path.join(previous_output_dir, 'models', 'all_rh_best_model.pth')
    ...
    # 获取之前的最佳验证损失
    best_val_loss = min(previous_train_history['val_losses'])
    counter = 0
    learning_rates = []
    ...
    try:
        for epoch in range(completed_epochs, config['num_epochs']):
            if time.time() - start_time > MAX_TRAINING_TIME:
                logging.info("Reached time limit, stopping training")
                break
            ...
            if val_loss < best_val_loss:
                best_val_loss = val_loss
                results_manager.save_model(model, 'all_rh_best_model')
                counter = 0
        ...
        # 1. 学习率变化详图
        plt.figure(figsize=(15, 5))
        plt.plot(train_history['learning_rates'])
        plt.title('Learning Rate Changes During Training')
        plt.xlabel('Training Steps')
        plt.ylabel('Learning Rate')
        plt.yscale('log')
        plt.grid(True)
        plt.savefig(os.path.join(results_manager.subdirs['plots']['training'], 
                                'step_detailed_learning_rate_schedule.png'))
        plt.close()

        # 2. 每epoch学习率
        plt.figure(figsize=(10, 5))
        combined_lr = learning_rates  # 包含两次训练的学习率
        plt.plot(range(len(combined_lr)), combined_lr)
        plt.title('Learning Rate Schedule')
        plt.xlabel('Training Steps')
        plt.ylabel('Learning Rate')
        plt.grid(True)
        ...
        plt.grid(True)
        plt.savefig(os.path.join(results_manager.subdirs['plots']['training'], 
                                'all_rh_learning_curve_predict_brainimages_improved.png'))
        plt.close()

        # 4. 评估和预测图
        logging.info("Starting final evaluation...")
        metrics = evaluate_model(model, test_loader, device, 
                               results_manager.subdirs['plots']['training'])
        save_attention_maps(model, test_loader, 
                          results_manager.subdirs['attention']['analysis'])
        ...
        final_results = {
            ...
            'metrics': metrics,
            'total_epochs': len(train_history['train_losses']),
            'best_val_loss': best_val_loss
        }
        np.save(os.path.join(results_manager.subdirs['logs'], 'final_results.npy'), 
                final_results)
        ...
        np.save(os.path.join(results_manager.subdirs['logs'], 'rh_training_config.npy'), config_info)