...
class ModelExperiments:
    def __init__(self, model_class, dataset_class, device, num_phenotypes=3, 
        ...
        if experiment_dir is None:
            self.results_dir = Path('/home/jouyang1/test_experiment_results_rh' if test_mode else '/home/jouyang1/experiment_results_primary_rh')
        else:
    ...
    def plot_scatter(self, predictions, targets, experiment_name, score_name, additional_info=None):
        ...
        plt.legend()
        save_path = self.vis_dir / f'rh_scatter_{experiment_name}_{score_name}.png'
        ...
        for epoch in range(self.base_epochs):
            ...
            if early_stopping:
                ...
                if metrics['patience_counter'] >= self.patience:
                    logging.info(f"rh_{experiment_name} - Early stopping triggered at epoch {epoch}")
                    break
            
            logging.info(f"{experiment_name} - Epoch {epoch+1}/{self.base_epochs}, "
                        f"rh: Train Loss: {avg_train_loss:.4f}, Val Loss: {avg_val_loss:.4f}")
    ...
    def sensitivity_test(self, model_path, test_loader, num_trials=5, noise_levels=[0.05, 0.1, 0.15]):
        ...
        Args:
            ...
            noise_levels: 噪声级别列表
        """
        logging.info("1.1 rh - Starting enhanced sensitivity test...")
        ...
        for noise_level in noise_levels:
            ...
            for i, score_name in enumerate(self.target_names):
                ...
                self.plot_scatter(
                    ...
                    true_targets[:, i],
                    f'rh_gaussian_noise_level_{noise_level}',
                    score_name,
                    f'rh: Noise Level: {noise_level}\nICC: {gaussian_icc:.3f}\np-value: {gaussian_p_value:.3f}'
                )
                
                self.plot_scatter(
                    np.mean(masking_predictions[:, :, i], axis=0),
                    true_targets[:, i],
                    f'rh_random_masking_level_{noise_level}',
                    score_name,
                    f'rh: Mask Ratio: {noise_level}\nICC: {masking_icc:.3f}\np-value: {masking_p_value:.3f}'
                )
                ...
                plt.title(f'rh_{score_name} - Gaussian Noise Impact (level={noise_level})')
                ...
                plt.title(f'rh_{score_name} - Random Masking Impact (level={noise_level})')
                plt.xticks(range(1, num_trials + 2), 
                          ['Original'] + [f'Trial {j+1}' for j in range(num_trials)])
                
                plt.tight_layout()
                plt.savefig(self.vis_dir / f'rh_noise_impact_boxplot_{score_name}_level_{noise_level}.png')
                plt.close()
        
        # 保存结果到JSON文件
        with open(self.metrics_dir / '1_1_rh_sensitivity_test_results.json', 'w') as f:
            json.dump({
                'test_parameters': {
        ...
        progress_results = {
            'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            'metrics_file': str(self.metrics_dir / '1_1_rh_sensitivity_test_results.json')
        }
    ...
    def sample_size_impact(self, train_data, val_data, sample_sizes=[0.25, 0.5, 0.75, 1.0]):
        ...
        Args:
            ...
            sample_sizes: 样本比例列表
        """
        logging.info("1.2 rh - Starting enhanced sample size impact analysis...")
        ...
        for size in sample_sizes:
            logging.info(f"rh: Testing sample size: {size*100}%")
            ...
            for repeat in range(self.experiment_repeats):
                logging.info(f"rh - Repeat {repeat + 1}/{self.experiment_repeats}")
                ...
                metrics = self._train_model(
                    ...
                    experiment_name=f"rh - sample_size_{size}_repeat_{repeat}"
                )
                ...
                for i, score_name in enumerate(self.target_names):
                    ...
                    self.plot_scatter(
                        current_preds,
                        current_targets,
                        f'rh - sample_size_{size}_repeat_{repeat}',
                        score_name,
                        f'rh - Sample Size: {size*100}%\nRepeat: {repeat+1}/{self.experiment_repeats}'
                    )
        ...
        # 保存结果
        with open(self.metrics_dir / '1_2_rh_sample_size_impact_results.json', 'w') as f:
            json.dump({
                'test_parameters': {
        ...
        progress_results = {
            'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            'metrics_file': str(self.metrics_dir / '1_2_rh_sample_size_impact_results.json'),
            'sample_sizes': sample_sizes
        }
    ...
    def _plot_learning_curves(self, results):
        ...
        for i, metric in enumerate(metrics):
            ...
            ax.set_title(f'rh - {metric.upper()} vs Sample Size')
        ...
        plt.tight_layout()
        plt.savefig(self.vis_dir / 'rh_learning_curves.png', dpi=300)
        plt.close()
    ...
    def stability_evaluation(self, train_data, n_splits=5):
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(train_data)):
            logging.info(f"rh - Processing fold {fold + 1}/{n_splits}")
            ...
            train_metrics = self._train_model(
                ...
                optimizer, 
                criterion,
                experiment_name=f"rh_stability_fold_{fold}"
            )
        ...
        # 保存结果
        with open(self.metrics_dir / '2_rh_stability_evaluation_results.json', 'w') as f:
            json.dump({
                'test_parameters': {
                    'n_splits': n_splits,
                },
        ...
        progress_results = {
            'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            'metrics_file': str(self.metrics_dir / '2_rh_stability_evaluation_results.json'),
            'n_splits': n_splits
        }
    ...
    #         ax.set_title(f'rh - {score_name} Training Curves')
    #         ax.set_xlabel('Epoch')
    #         ax.set_ylabel('Loss')
    #         ax.grid(True, alpha=0.3)
    #         ax.legend()
    ...
    #     plt.tight_layout()
    #     plt.savefig(self.vis_dir / 'rh_training_curves.png', dpi=300)
    #     plt.close()
    ...
    def _plot_performance_curves(self, curves_data, experiment_name):
        ...
        plt.title('rh - Model Training Stability')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.grid(True, alpha=0.3)
        plt.legend()
        
        plt.savefig(self.vis_dir / f'rh_performance_curves_{experiment_name}.png', dpi=300)
        plt.close()
    ...
    def _plot_stability_boxplots(self, results):
        ...
        for i, metric in enumerate(metrics):
            ...
            axes[i].set_title(f'rh - {metric.upper()} Distribution')
            axes[i].set_xticklabels(self.target_names)
            axes[i].grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(self.vis_dir / 'rh_stability_boxplots.png', dpi=300)
        plt.close()
    ...
    def computational_assessment(self, model_path, train_loader, test_loader):
        ...
        # 保存结果
        with open(self.metrics_dir / '3_rh_computational_assessment_results.json', 'w') as f:
        ...
        progress_results = {
            'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            'metrics_file': str(self.metrics_dir / '3_rh_computational_assessment_results.json'),
            'model_path': str(model_path)
        }
    ...
    def _plot_memory_metrics(self, results):
        ...
        plt.title('rh - Time Performance (seconds)')
        plt.xticks(rotation=45)
        plt.grid(True, alpha=0.3)
        ...
        plt.title('rh - GPU Memory Usage Details (GB)')
        plt.xticks(rotation=45)
        plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(self.vis_dir / 'rh_performance_metrics.png')
        plt.close()
